...
*/	
	...
	//new block fora2323
	private final ArrayList<String> onCreateTopCode = new ArrayList<>();
	private final ArrayList<String> onCreateBottomCode = new ArrayList<>();
	private String customExtendsActivity = null;
	private String customExtendsFragment = null;
		...
		for (Map.Entry<String, ArrayList<BlockBean>> blocks : jC.a(projectDataManager.a).b(projectFileBean.getJavaName()).entrySet()) {
			for (BlockBean block : blocks.getValue()) {
				switch (block.opCode) {
					...
					break;
					case "addOnCreateCodeTop":
					if (!block.parameters.get(0).trim().isEmpty()) {
						onCreateTopCode.add(block.parameters.get(0));
					}
					break;
					case "addOnCreateCodeBottom":
					if (!block.parameters.get(0).trim().isEmpty()) {
						onCreateBottomCode.add(block.parameters.get(0));
					}
					break;
					case "replaceExtendsActivity":
					if (!block.parameters.get(0).trim().isEmpty()) {
						customExtendsActivity = block.parameters.get(0).trim();
					}
					break;
					case "replaceExtendsFragment":
					if (!block.parameters.get(0).trim().isEmpty()) {
						customExtendsFragment = block.parameters.get(0).trim();
					}
					break;    
				}
			}
...
*/	
	public String generateCode(boolean isAndroidStudioExport, String sc_id) {
		boolean isDialogFragment = (projectFileBean.fileName.contains("_dialog_fragment") || DayDreamProjectSettings.getActivityType(Configs.currentProjectID, projectFileBean.fileName).contains("_dialog_fragment"));
		
		boolean isBottomDialogFragment = (projectFileBean.fileName.contains("_bottomdialog_fragment") || DayDreamProjectSettings.getActivityType(Configs.currentProjectID, projectFileBean.fileName).contains("_bottomdialog_fragment"));
		boolean isFragment = (projectFileBean.fileName.contains("_fragment") || DayDreamProjectSettings.getActivityType(Configs.currentProjectID, projectFileBean.fileName).contains("_fragment"));
		...
		if (isFragment) {
			if (customExtendsFragment != null) {
				sb.append(customExtendsFragment);
			...
			if (customExtendsActivity != null) {
				sb.append(customExtendsActivity);
			} else if (buildConfig.g) {
		...
		if (isFragment) {
			...
			for (String value : onCreateTopCode) {
				if (!value.isEmpty()) {
		...
		if (!isFragment) {
			...
			for (String value : onCreateBottomCode) {
				if (!value.isEmpty()) {
					sb.append(value).append(EOL);
				}
			}
	...
	private void handleAppCompat() {
		...
		if (ProjectLibrary.isEnabledAppCompat(Configs.currentProjectID)) {
			...
			if (DayDreamProjectSettings.isEnableDayDream(Configs.currentProjectID)) {
				...
				if (DRFeatureManager.isShizukuEnabled(Configs.currentProjectID, projectFileBean.fileName)) {
					addImport("rikka.shizuku.Shizuku");
				}
		...
		//imports that have never been used
