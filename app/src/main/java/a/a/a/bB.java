    ...
    public static Toast a(Context context, CharSequence charSequence, int duration, int gravity, float xOffset, float yOffset, int toastType) {
        try {
            ...
            if (toastType == TOAST_WARNING) {
                int errorColor = ThemeUtils.getColor(themedContext, R.attr.colorError);
                int errorContainer = ThemeUtils.getColor(themedContext, R.attr.colorErrorContainer);

                binding.cardView.setCardBackgroundColor(errorContainer);
                binding.cardView.setStrokeColor(errorColor);
                binding.tvStoast.setTextColor(ThemeUtils.getColor(themedContext, R.attr.colorOnErrorContainer));
            } else {
                int primaryColor = ThemeUtils.getColor(themedContext, R.attr.colorPrimary);
                int surfaceContainer = ThemeUtils.getColor(themedContext, R.attr.colorSurfaceContainer);

                binding.cardView.setCardBackgroundColor(surfaceContainer);
                binding.cardView.setStrokeColor(primaryColor);
                binding.tvStoast.setTextColor(primaryColor);
            }
